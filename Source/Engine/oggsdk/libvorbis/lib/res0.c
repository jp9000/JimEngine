...
vorbis_info_residue *res0_unpack(vorbis_info *vi,oggpack_buffer *opb){
  ...
  if(info->groupbook>=ci->books)goto CrashError;
  for(j=0;j<acc;j++)
    if(info->booklist[j]>=ci->books)goto CrashError;
  ...
  if(n>0){
	...
	if(s==0){
	  ...
	  for(j=0;j<ch;j++){
	    ...
	    if(partword[j][l]==NULL)goto CrashError;
	  }
	}
...
int res2_inverse(vorbis_block *vb,vorbis_look_residue *vl,
  ...
  if(n>0){
	...
	if(s==0){
	  ...
	  if(partword[l]==NULL)goto CrashError;
	}
